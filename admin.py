...
class CompanyAdmin(BaseAdmin):
    ...
    def country_view(
        self, request, action, object_id=None, form_url=None, extra_context=None
    ):
        extra_context = extra_context or {}
        extra_context['action'] = action
        countries = import_string(f'company.backends.{action}_backends')
        extra_context['countries'] = {
            key: get_country_by(key, 'alpha2') for key in countries
        }
        return self.admincustom_view(
            request,
            object_id,
    ...
    create_admin_path = (
        'actions/create/countries/<str:country>/<str:rna_or_siren>/'
    )
    ...
    def create_view(
        ...
        if (
            request.method == 'POST'
            and request.POST.get('rna_or_siren') == rna_or_siren
        ):
    ...
    getorcreate_admin_path = (
        'actions/getorcreate/countries/<str:country>/<str:rna_or_siren>/'
    )
    getorcreate_admin_suffix = 'getorcreate'

    def getorcreate_view(
        self,
        request,
        country,
        rna_or_siren,
        ...
        try:
            entity = CompanyModel.objects.get(
                Q(company_fr__rna=rna_or_siren)
                | Q(company_fr__siren=rna_or_siren)
            )
            return redirect(
                f'admin:{self.model._meta.app_label}_{self.model._meta.model_name}_change',
                object_id=entity.id,
    ...
    def get_urls(self):
        ...
        my_urls = [
            path(
                ...
                self.wrap(
                    self.action_view, object_tools=self.action_object_tools
                ),
            ...
            path(
                ...
                self.wrap(
                    self.country_view, object_tools=self.country_object_tools
                ),
            ...
            path(
                ...
                self.wrap(
                    self.search_view, object_tools=self.search_object_tools
                ),
