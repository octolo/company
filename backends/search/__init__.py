...
from company.choices import fr as choices  # noqa
...
class SearchBackend:
    ...
    def retry_with_backoff(
        self, operation, exceptions=(Exception,), return_on_failure=None
    ):
        retries = 0
        while self.retries < self.max_retries:
            try:
                ...
                self._logger.info(
                    'Operation failed (attempt %d). Retrying in %.2f seconds... (exception: %s)',
                    retries,
                    delay_with_jitter,
                    e,
                )
    ...
    def get_value(self, obj, key, name=None, default=None):
        ...
        if key in self.fields_association and isinstance(
            self.fields_association[key], tuple
        ):
            for field in self.fields_association[key]:
                ...
                if value:
                    return value
            return default
        return self.getattr_recursive(
            obj, self.fields_association.get(key, key)
        )
    ...
    def search(self, search, *args, **kwargs):
        total, results = self.retry_with_backoff(
            lambda: self.get_results(search, *args, **kwargs),
            return_on_failure=(0, []),
        )
